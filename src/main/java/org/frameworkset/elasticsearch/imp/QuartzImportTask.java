...
//		importBuilder.setStatusTableDML(DBConfig.mysql_createStatusTableSQL);
			...
			mongoDBInputConfig.setName("session").setDb("sessiondb")
...
//		//设置任务执行拦截器结束，可以添加多个
			//增量配置开始
			importBuilder.setLastValueColumn("lastAccessedTime");//手动指定数字增量查询字段，默认采用上面设置的sql语句中的增量变量名称作为增量查询字段的名称，指定以后就用指定的字段
