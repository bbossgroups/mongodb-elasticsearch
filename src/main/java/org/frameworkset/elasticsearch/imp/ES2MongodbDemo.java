 ...
 * @version 1.0
 */
public class ES2MongodbDemo {
	private static Logger logger = LoggerFactory.getLogger(ES2MongodbDemo.class);
	public static void main(String args[]){
		ES2MongodbDemo dbdemo = new ES2MongodbDemo();
	...
	public void scheduleTimestampImportData(boolean dropIndice){
		...
		mongoDBOutputConfig.setMultiCollections(false);
...
//					 .setScheduleDate(date) //指定任务开始执行时间：日期
		...
		importBuilder.setLastValueStorePath("es2mongodb_import");//记录上次采集的增量字段值的文件路径，作为下次增量（或者重启后）采集数据的起点，不同的任务这个路径要不一样
